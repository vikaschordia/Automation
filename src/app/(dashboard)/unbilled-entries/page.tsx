...
export default function UnbilledEntriesPage() {
  ...
  const { data, isLoading } = useUnbilledEntries(cursor.year, cursor.month);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button asChild variant="outline">
            <a href={`/api/unbilled-entries/export?year=${cursor.year}&month=${cursor.month}`}>
            ...
            {!isLoading && entries.length === 0 && (
              <TableRow>
                <TableCell colSpan={columnCount} className="py-12 text-center text-sm text-muted-foreground">
                  ...
                  No entries for {MONTH_NAMES[cursor.month - 1]} {cursor.year}.{canManage && " Add one to get started."}
                </TableCell>
              </TableRow>
            )}
            {entries.map((e) => (
      ...
      {canManage && (
        <>
          <EntryFormDialog open={formOpen} onOpenChange={setFormOpen} entry={editing} year={cursor.year} month={cursor.month} />
