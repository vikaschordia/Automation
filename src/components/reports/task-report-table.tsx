...
const columns = [
  ...
  columnHelper.accessor((row) => row.department.name, { id: "department", header: "Department", size: 140 }),
  ...
  columnHelper.accessor("delayDays", {
    ...
    cell: (info) => {
      ...
      return days > 0 ? <span className="font-medium text-red-600">{days}d</span> : "—";
    },
  }),
];

const SORTABLE_COLUMN_IDS = ["title", "employee", "company", "department", "priority", "status", "dueDate", "completedDate"];

export function TaskReportTable({ tasks, isLoading, sort, onSort }: { tasks: TaskRow[]; isLoading: boolean; sort: SimpleSort; onSort: (key: string) => void }) {
            ...
            table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="border-b">
                {row.getVisibleCells().map((cell) => (
                  <td key={cell.id} style={{ width: cell.column.getSize() }} className="h-11 px-3 align-middle">
